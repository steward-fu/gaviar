...
static void sun6i_dma_free_desc(struct virt_dma_desc *vd)
{
	...
	while (v_lli) {
		...
		dma_pool_free(sdev->pool, v_lli, v_lli->this_phy);
		...
		unsigned long flags, void *context)
{
	...
	dma_addr_t p_lli, src_tmp, dst_tmp;
	u32 lli_cfg;
	int i, ret;
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		if (dir == DMA_MEM_TO_DEV) {
			src_tmp = sg_dma_address(sg);
			dst_tmp = sconfig->dst_addr;
			v_lli->src = (u32)src_tmp;
			v_lli->dst = (u32)dst_tmp;
			v_lli->cfg = lli_cfg;
			...
			src_tmp = sconfig->src_addr;
			dst_tmp = sg_dma_address(sg);
			v_lli->src = (u32)src_tmp;
			v_lli->dst = (u32)dst_tmp;
			v_lli->cfg = lli_cfg;
		...
		v_lli->para = SET_DST_HIGH_ADDR(dst_tmp)
			| SET_SRC_HIGH_ADDR(src_tmp)
			| NORMAL_WAIT;
