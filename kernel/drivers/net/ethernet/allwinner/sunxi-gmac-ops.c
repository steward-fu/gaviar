...
#define RX_TH			0x0030
#define RX_FLOW_CTL_MASK	0x1000000
...
int sunxi_read_rx_flowctl(void *iobase)
{
	unsigned int value;

	value = readl(iobase + GETH_RX_CTL1);

	return value & RX_FLOW_CTL_MASK;
}

int sunxi_read_tx_flowctl(void *iobase)
{
	unsigned int value;

	value = readl(iobase + GETH_TX_FLOW_CTL);

	return value & TX_FLOW_CTL_MASK;
}

void sunxi_write_rx_flowctl(void *iobase, bool flag)
{
	unsigned int value;
...
void sunxi_write_tx_flowctl(void *iobase, bool flag)
{
	unsigned int value;
	...
	if (flag)
		value |= TX_FLOW_CTL_MASK;
	else
		value &= ~TX_FLOW_CTL_MASK;

	writel(value, iobase + GETH_TX_FLOW_CTL);
}
