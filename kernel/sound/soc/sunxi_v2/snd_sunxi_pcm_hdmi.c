...
static struct sunxi_pcm sunxi_pcm = {
				   ...
				   struct snd_pcm_hw_params *params)
{
	...
	sunxi_pcm.hdmi_fmt = snd_sunxi_hdmi_get_fmt();
	if (sunxi_pcm.hdmi_fmt > HDMI_FMT_PCM) {
...
pcm_rawdata_mode:
	...
	sunxi_pcm.raw_dma_area = dma_alloc_coherent(dev,
					(params_buffer_bytes(params) * 2),
					&(sunxi_pcm.raw_dma_addr), GFP_KERNEL);
	if (sunxi_pcm.raw_dma_area == NULL) {
		SND_LOG_ERR(HLOG, "pcm rawdata mode get mem failed\n");
		return -ENOMEM;
	}
	sunxi_pcm.pcm_dma_addr = substream->dma_buffer.addr;
	substream->dma_buffer.addr = (dma_addr_t)(sunxi_pcm.raw_dma_addr);
...
static int sunxi_pcm_hw_free_raw(struct snd_pcm_substream *substream)
{
	...
	if (snd_pcm_lib_buffer_bytes(substream) && (sunxi_pcm.hdmi_fmt > HDMI_FMT_PCM)) {
		dma_free_coherent(dev,
				  (snd_pcm_lib_buffer_bytes(substream) * 2),
				  sunxi_pcm.raw_dma_area,
				  sunxi_pcm.raw_dma_addr);
		substream->dma_buffer.addr = sunxi_pcm.pcm_dma_addr;
		sunxi_pcm.raw_dma_area = NULL;
	}
...
static int sunxi_pcm_prepare_raw(struct snd_pcm_substream *substream)
{
	...
	if (sunxi_pcm.hdmi_fmt > HDMI_FMT_PCM) {
		if (sunxi_pcm.change_size_flag) {
			runtime->buffer_size = sunxi_pcm.buffer_size;
			runtime->period_size = sunxi_pcm.period_size;
		} else {
			sunxi_pcm.change_size_flag = true;
			runtime->buffer_size *= 2;
			runtime->period_size *= 2;
			sunxi_pcm.buffer_size = runtime->buffer_size;
			sunxi_pcm.period_size = runtime->period_size;
		}
	} else {
		if (sunxi_pcm.change_size_flag) {
			sunxi_pcm.change_size_flag = false;
			runtime->buffer_size = sunxi_pcm.buffer_size / 2;
			runtime->period_size = sunxi_pcm.period_size / 2;
		}
	}
			      ...
			      unsigned long bytes)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (sunxi_pcm.hdmi_fmt > HDMI_FMT_PCM) {
			char *hdmihw_area = sunxi_pcm.raw_dma_area + 2 * hwoff;
			snd_sunxi_transfer_format_61937_to_60958(
				(int *)hdmihw_area, (short *)hwbuf,
				bytes, runtime->rate, sunxi_pcm.hdmi_fmt);
		}
